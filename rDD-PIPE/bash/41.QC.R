...
LIB=args[1]
#LIB="rHG008"
#setwd("//Users/zhongyuantian/macshare/workSpace2021B/script/rDDPP/rHG008")
IN=paste0(LIB,".final_stats.tsv")
ION=paste0(LIB,".e500.clusters.cis.chiasig.gz")
...
P1<-ggplot(DF00) + 
  ...
  geom_text(aes(x=Total_read_pairs/2,y=-0.5,label = paste0("Total_read_pairs ",scales::comma(Total_read_pairs)," (",round(1*100), "%)")),size=TS,color="white")+
#  geom_text(aes(x=Total_read_pairs/2,y=-0.5,label = paste0("Total_read_pairs: ",scales::comma(Total_read_pairs))),size=TS,color="white")+
  
  geom_rect(aes(xmin=0,xmax=Read_pairs_with_linker,ymin=-1.9,ymax=-1),fill="navy")+
  geom_text(aes(x=Read_pairs_with_linker/2,y=-1.5,label = paste0("Read_pairs_with_linker ",scales::comma(Read_pairs_with_linker)," (",round(Read_pairs_with_linker / Total_read_pairs * 100), "%)")),size=TS,color="white")+
...
#                label = paste0("Read_pairs\nno_linker ",round((Total_read_pairs-Read_pairs_with_linker) / Total_read_pairs * 100), "%")),size=TS,color="white")+
  geom_rect(aes(xmin=0,xmax=PET,ymin=-2.9,ymax=-2),fill="navy")+
  geom_text(aes(x=PET/2,y=-2.5,label = paste0("Two_tags (PET) ",scales::comma(PET)," (",round(PET / Read_pairs_with_linker * 100), "%)")),size=TS,color="white")+
  geom_rect(aes(xmin=PET,xmax=PET+One_tag,ymin=-2.9,ymax=-2),fill="#9E9D24")+
  geom_text(aes(x=PET+One_tag/2,y=-2.5,label = paste0("One_tag ",scales::comma(One_tag)," (",round(One_tag / Read_pairs_with_linker * 100), "%)")),size=TS,color="white")+
  #geom_rect(aes(xmin=PET+One_tag,xmax=PET+One_tag+No_tag,ymin=-2.9,ymax=-2),fill="gray30")+
 # geom_text(aes(x=PET+One_tag+No_tag/2,y=-2.2,label = paste0("No_tags ",round( (Read_pairs_with_linker-PET-One_tag) / Read_pairs_with_linker * 100), "%")),size=TS,color="white")+
  geom_rect(aes(xmin=0,xmax=Uniquely_mapped_PET,ymin=-3.9,ymax=-3),fill="navy")+
 # geom_text(aes(x=Uniquely_mapped_PET,y=-3.5,label = paste0("Uniquely_mapped_PET ",scales::comma(Uniquely_mapped_PET)," (",round(Uniquely_mapped_PET / PET * 100), "%)")),size=TS,color="black")+
  geom_text(aes(x=Uniquely_mapped_PET,y=-3.5,label = paste0("Uniquely_mapped_PET ",scales::comma(Uniquely_mapped_PET)," (",round(Uniquely_mapped_PET / PET * 100), "%)"),hjust=-0.05),size=TS,color="black")+
  geom_rect(aes(xmin=0,xmax=Non_redundant_PET,ymin=-4.9,ymax=-4),fill="navy")+
  geom_text(aes(x=Non_redundant_PET,y=-4.5,label = paste0("Non_redundant_PET ",scales::comma(Non_redundant_PET)," (",round(Non_redundant_PET / Uniquely_mapped_PET * 100), "%)"),hjust=-0.05),size=TS,color="black")+
 ...
 # geom_rect(aes(xmin=Inter_ligation_PET,xmax=Inter_ligation_PET+Self_ligation_PET,ymin=-5.9,ymax=-5),fill="gray30")+
  geom_text(aes(x=Inter_ligation_PET,y=-5.5,label = paste0("Inter_ligation_PET ",scales::comma(Inter_ligation_PET)," (",round(Inter_ligation_PET / Non_redundant_PET * 100), "%)"),hjust=-0.05),size=TS,color="black")+
#  geom_text(aes(x=Inter_ligation_PET+Self_ligation_PET/2,y=-5.5,label = paste0("Self_ligation\nPET ",round(Self_ligation_PET / Non_redundant_PET * 100), "%")),size=TS,color="white")+
 ...
 # geom_rect(aes(xmin=PET_4loop,xmax=PET_4loop+Singleton,ymin=-6.9,ymax=-6),fill="gray30")+
  geom_text(aes(x=(PET_4loop),y=-6.5,label = paste0("PET_for_cluster ",scales::comma(PET_4loop)," (",round(PET_4loop / Inter_ligation_PET * 100), "%)"),hjust=-0.05),size=TS,color="black")+
 # geom_text(aes(x=PET_4loop+Singleton/2,y=-6.5,label = paste0("Singleton ",round(Singleton / (PET_cluster+Singleton) * 100), "%")),size=TS,color="white")+
  

 # scale_x_log10(labels=f2si,breaks=pretty_breaks())+
  labs(x="read count (log)",y="")+
...
P2<-ggplot(DF00) + 
  ...
  geom_text(aes(x=1/2,y=-0.5,label = paste0("PET_cluster ",scales::comma(PET_cluster)," (",round(100), "%)")),size=TS,color="#ffffff")+
  
  geom_rect(aes(xmin=0,xmax=(Intra_chr_PET_cluster/PET_cluster)/1,ymin=-1.9,ymax=-1),fill="#3e2723")+
  geom_text(aes(x=Intra_chr_PET_cluster/PET_cluster/2,y=-1.5,label = paste0("Intra_chr_cluster ",scales::comma(Intra_chr_PET_cluster)," (",round(Intra_chr_PET_cluster/PET_cluster*100), "%)"),hjust=0.5),size=TS,color="#ffffff")+
  
  geom_rect(aes(xmin=Intra_chr_PET_cluster/PET_cluster,xmax=1,ymin=-1.9,ymax=-1),fill="darkred")+
  geom_text(aes(x=Intra_chr_PET_cluster/PET_cluster+Inter_chr_PET_cluster/PET_cluster/2,y=-1.5,label = paste0("Inter_chr_cluster ",scales::comma(Inter_chr_PET_cluster)," (",round(Inter_chr_PET_cluster/PET_cluster*100), "%)")),size=TS,color="#ffffff")+
  
 # geom_text(aes(x=Intra_chr_PET_cluster,y=-1.5,label = paste0("intra_chr_cluster/inter_chr_cluster\nratio ",round(Inter_chr_PET_cluster/PET_cluster*100), "%")),size=TS,color="white")+
  
  geom_rect(aes(xmin=0,xmax=Host_Host_Loops/PET_cluster,ymin=-2.9,ymax=-2),fill="#006064")+
  geom_text(aes(x=Host_Host_Loops/PET_cluster/2,y=-2.5,label = paste0("Host_Host_Loops ",scales::comma(Host_Host_Loops)," (",round(Host_Host_Loops/PET_cluster*100), "%)")),size=TS,color="#ffffff")+
  
  geom_rect(aes(xmin=Host_Host_Loops/PET_cluster,xmax=1,ymin=-2.9,ymax=-2),fill="#4e342e")+
  geom_text(aes(x=Host_Host_Loops/PET_cluster*0.90+HE_EE_loops/PET_cluster/2,y=-2.5,label = paste0("HE&EE_loops ",scales::comma(HE_EE_loops)," (",round(HE_EE_loops/PET_cluster*100), "%)-->")),size=TS,color="#ffffff")+
#  scale_x_log10(labels=f2si,breaks=pretty_breaks())+
  scale_x_continuous(position = "top")+
...
FIOON=read.table(gzfile(ION),header=TRUE)
...
pdf(PLOT_NAME,18,16)
#png("STA.png",1800,1600)
library(gridExtra)
grid.arrange(P1,P2,arrangeGrob(PLOT_ECDF,PLOT_DEN,nrow=2,widths=c(5,7)),nrow=3,heights=c(5,3,8),top=PLOT_NAME)
dev.off()
